...
impl World {
    pub fn evolve_until_stop(&self, initial_ray: Photon3, debug: bool) -> (WorldPhoton3State<'_>, StopReason) {
        let mut ray = self.manifold.to_photon4(initial_ray);

        for _ in 0..MAX_STEPS {
            ray = self.manifold.step_along_null_geodesic(ray);

            if debug {
                let world_ray = self.manifold.to_photon3(ray);
            ...
            if self.manifold.is_singular(ray.pos) {
                return (
                    WorldPhoton3State {
                        photon3: self.manifold.to_photon3(ray),
                        normal: None,
                        material: None,
                    },
            ...
            for obj in &self.objects {
                if let Some(hit) = obj.hit(&self.manifold.to_photon3(ray)) {
