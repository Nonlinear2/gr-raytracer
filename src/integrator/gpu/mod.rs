...
impl GpuIntegrator {
    ...
    fn create_buffers(&self, packed_rays: &[PackedPhoton3], rays_byte_size: u64) -> Buffers {
        ...
        let objects_buffer = self.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("objects"),
            contents: bytemuck::cast_slice(&self.packed_objects),
            usage: wgpu::BufferUsages::STORAGE,
        });
        ...
        let trace_readback = self.device.create_buffer(&wgpu::BufferDescriptor {
    ...
    fn create_bind_group(&self, buffers: &Buffers) -> wgpu::BindGroup {
        let entries = vec![
            wgpu::BindGroupEntry { binding: 0, resource: buffers.input.as_entire_binding() },
    ...
    fn dispatch_and_readback(
        ...
        encoder.copy_buffer_to_buffer(&buffers.output, 0, &buffers.readback, 0, buffer_size);

        let trace_byte_size = std::mem::size_of::<PackedTraceResult>() as u64;
        if DEBUG {
            encoder.copy_buffer_to_buffer(&buffers.trace_output, 0, &buffers.trace_readback, 0, trace_byte_size);
        }
        ...
        let trace_slice = buffers.trace_readback.slice(..);

        let (_trace_sender, trace_receiver) = 
            if DEBUG {
...
impl GeodesicIntegrator for GpuIntegrator {
    fn run(&self, rays: Vec<Photon3>) -> (Vec<Color>, Option<Vec<PackedTraceResult>>) {
        ...
        let bind_group = self.create_bind_group(&buffers);
